 ...
 *
 * Architecture :
 *  - react-hook-form + zodResolver(seasonTournamentCreateSchema)
...
import { createSeasonTournamentAction } from '../saisons/[id]/tournois/nouveau/actions'
...
type FormInput = z.input<typeof seasonTournamentCreateSchema>
type FormOutput = z.output<typeof seasonTournamentCreateSchema>

interface TournamentInSeasonFormProps {
  seasonId: string
 ...
 * valides pour seasonTournamentCreateSchema. Tolérant aux defaults absents.
 */
function buildInitialValues(
  ...
  return {
    name: '',
    season_id: seasonId,
    tournament_number: suggestedNumber,
    start_date: '',
    end_date: '',
    capacity: 32,
...
// Composant principal
// ===========================================================================
export function TournamentInSeasonForm({
  seasonId,
  suggestedNumber,
  defaults,
  eventLocation,
}: TournamentInSeasonFormProps) {
  const router = useRouter()
  const [isPending, startTransition] = useTransition()

  const form = useForm<FormInput, unknown, FormOutput>({
    resolver: zodResolver(seasonTournamentCreateSchema),
    defaultValues: buildInitialValues(seasonId, suggestedNumber, defaults, eventLocation),
    mode: 'onBlur',
  })

  const onSubmit = (values: FormOutput) => {
    startTransition(async () => {
      const result = await createSeasonTournamentAction(values)
      if (result.success) {
        toast.success('Tournoi de saison créé.')
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="md:col-span-2">
                  ...
                  <FormDescription>
                    Numéro suggéré automatiquement (modifiable).
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
